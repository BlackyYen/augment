...
def flip_xml(flip_img_name, xml_tree, flip_type):
    ...
    size = root.find('size')
    imgw, imgh, depth = int(size.find('width').text), int(
        size.find('height').text), int(size.find('depth').text)
...
def flip_img_xml_from_dir(imgs_dir,xmls_dir,imgs_save_dir,xmls_save_dir,img_suffix,name_suffix,\
    ...
    for root, dirs, files in os.walk(xmls_dir):
        for xml_name in files:
            ...
            for tp in types:
                flip_img_name = xml_name.split(
                    '.')[0] + '_' + name_suffix + '_type' + str(
                        tp) + '.' + img_file.split('.')[-1]
                imgflip, xmlflip = flip_img_xml(img,
                                                voc_xml.get_xml_tree(xml_file),
                                                flip_img_name, tp)
                cv2.imwrite(os.path.join(imgs_save_dir, flip_img_name),
                            imgflip)
                xmlflip.save_xml(xmls_save_dir,
                                 flip_img_name.split('.')[0] + '.xml')


def main():
    imgs_dir = r'D:\dataset\yolo-sperm\org\JPEGImages'
    xmls_dir = r'D:\dataset\yolo-sperm\org\Annotations'

    imgs_save_dir = r'D:\dataset\yolo-sperm\aug\JPEGImages-flip'
    if not os.path.exists(imgs_save_dir):
        os.makedirs(imgs_save_dir)
    xmls_save_dir = r'D:\dataset\yolo-sperm\aug\Annotations-flip'
    ...
    flip_img_xml_from_dir(imgs_dir, xmls_dir, imgs_save_dir, xmls_save_dir,
                          img_suffix, name_suffix, flip_types, random_flip)
